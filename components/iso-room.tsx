...
import type { GameObject, GameState } from "@/state/types"
...
export default function IsoRoom({
  ...
  const interactRef = useRef<{ id: string; name: string; x: number; yHead: number }[]>([])
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      // Check for interactions with furniture/music boxes first
      const hit = interactRef.current.find((t) => px >= t.x && px <= t.x + 40 && py >= t.yHead && py <= t.yHead + 60)
      if (hit && hit.id.startsWith("music")) {
        onMusicBoxToggle?.(hit.id.substring(6))
        const mb = getMusicBoxes(room).find((m) => m.id === hit.id.substring(6))
        if (mb) {
      ...
      if (gameObjects.length > 0 && gameState && onInteract) {
        ...
        const obj = gameObjects.find((o) => Math.round(o.x) === Math.round(tx) && Math.round(o.y) === Math.round(ty))
        if (obj && nearbyObject?.id === obj.id) {
          onInteract(obj.id)
          return
        }
      }
  ...
  useEffect(() => {
    const down = (e: KeyboardEvent) => {
      ...
      if (e.key === "e" && nearbyObject && onInteract) {
        e.preventDefault()
        onInteract(nearbyObject.id)
        return
      }
  ...
  }, [tryMoveOne, nearbyObject, onInteract])
  ...
  useEffect(() => {
    ...
    function loop(now: number) {
      ...
      // Draw game objects or furniture based on context
      if (gameObjects.length > 0 && gameState) {
        drawGameObjects(ctx, gameObjects, gameState, params, tileW, tileH, animRef.current)
        drawRoomDecorations(ctx, room, params, tileW, tileH, animRef.current)
      } else {
        // Original furniture for Habbo mode
        interactRef.current = []
        drawFurniture(
          ctx,
          room,
      ...
      const labels: { id: string; name: string; x: number; yHead: number }[] = []
      for (const { p, s } of peersSorted) {
        ...
        const headY = pY - 26
        labels.push({ id: p.id, name: p.name || "Guest", x: pX, yHead: headY })
      ...
      labels.push({ id: "self", name: selfName, x: ax, yHead: selfHeadY })
      ...
      hoverTargetsRef.current = labels.map((l) => ({ id: l.id, name: l.name, x: l.x, yHead: l.yHead }))

      if (nearbyObject) {
        drawInteractionPrompt(ctx, ax, ay - 40, nearbyObject.name)
      }
...
function drawFurniture(
  ...
  const musicBox = (m: { id: "sunny" | "night"; x: number; y: number; label: string }) => {
    ...
    opt.interactiveOut.push({ type: "music", id: `musicbox_${m.id}`, rect: { x: px - 18, y: py - 56, w: 36, h: 62 } })
    ctx.font = "700 10px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto"
    ctx.fillStyle = "#111827"
...
function drawNameplates(
  ctx: CanvasRenderingContext2D,
  labels: { id: string; name: string; x: number; yHead: number }[],
) {
  ...
  for (const l of labels) {
    ...
    while (tries < 8) {
      ...
      if (!placed.some((r) => intersects(candidate, r))) {
        ...
        ctx.fillStyle = "#111827"
        ctx.fillText(l.name, candidate.x + padX, candidate.y + h - padY - 2)
        break
      }
      cy -= 8
      tries++
...
function drawGameObjects(
  ...
  for (const obj of sorted) {
    const { px, py } = projectIso(obj.x, obj.y, params)

    const objState = gameState.objects[obj.id] || obj
    ...
    ctx.strokeText(obj.name || obj.id, px, py - 60)
    ctx.fillText(obj.name || obj.id, px, py - 60)
    ctx.textAlign = "left"
    ctx.restore()
    ...
    ctx.beginPath()
    ctx.arc(px, py, tileW * 1.5, 0, Math.PI * 2)
    ctx.fill()
    ctx.restore()

    switch (obj.type) {
      case "switch":
        drawSwitch(ctx, px, py, objState.state === "on", obj.id, gameState)
        break
      case "valve":
        drawValve(ctx, px, py, objState.state === "on", t)
        break
      case "door":
        drawDoor(ctx, px, py, tileW, tileH, objState.open === true, obj.orientation || "vertical")
        break
      case "led":
        drawLED(ctx, px, py, objState.state === "green" || objState.state === "on", obj.color || "red")
        break
      case "panel":
        drawPanel(ctx, px, py, objState.state === "activated", obj.label || "")
        break
      case "console":
        drawConsole(ctx, px, py, objState.correct === true)
        break
      case "weightPlate":
        drawWeightPlate(ctx, px, py, tileW, tileH, objState.state === "pressed")
        break
      case "item":
        if (!objState.carriedBy) {
          drawItem(ctx, px, py, obj.itemType || "key")
        }
        break
    }

    if (obj.type !== "led" && obj.type !== "item") {
      ctx.save()
      const pulse = (Math.sin(t * 3) + 1) / 2
      ctx.globalAlpha = 0.2 + pulse * 0.1
      ctx.fillStyle = objState.state === "on" || objState.open ? "#22c55e" : "#fbbf24"
      ctx.beginPath()
      ctx.arc(px, py - 20, 15 + pulse * 5, 0, Math.PI * 2)
      ctx.fill()
      ctx.restore()
    }
  }
...
function drawRoomDecorations(
  ...
  // Control Room (R1) - Technical equipment, monitors, cables
  if (roomId === "R1") {
    // Computer monitors on walls
    const monitor1 = projectIso(1, 2, params)
    rect(ctx, monitor1.px - 15, monitor1.py - 35, 30, 20, "#1e293b")
    rect(ctx, monitor1.px - 13, monitor1.py - 33, 26, 16, "#0ea5e9")

    const monitor2 = projectIso(10, 6, params)
    rect(ctx, monitor2.px - 15, monitor2.py - 35, 30, 20, "#1e293b")
    rect(ctx, monitor2.px - 13, monitor2.py - 33, 26, 16, "#22c55e")

    // Cable conduits
    ctx.strokeStyle = "#64748b"
    ctx.lineWidth = 2
    ctx.beginPath()
    ctx.moveTo(monitor1.px, monitor1.py - 15)
    ctx.lineTo(monitor1.px + 20, monitor1.py - 10)
    ctx.stroke()

    // Warning signs
    ctx.font = "700 12px ui-sans-serif, system-ui"
    ctx.fillStyle = "#fbbf24"
    ctx.fillText("⚠", monitor1.px - 25, monitor1.py - 40)
    ctx.fillText("⚠", monitor2.px + 20, monitor2.py - 40)
  }

  // Filtration Zone (R2) - Pipes, filters, industrial equipment
  else if (roomId === "R2") {
    // Large pipes running across ceiling
    for (let i = 0; i < 3; i++) {
      const pipe = projectIso(4 + i * 3, 2, params)
      ctx.fillStyle = "#475569"
      ctx.fillRect(pipe.px - 25, pipe.py - 50, 50, 8)
      ctx.strokeStyle = "#000"
      ctx.strokeRect(pipe.px - 25, pipe.py - 50, 50, 8)

      // Pipe joints
      circle(ctx, pipe.px - 20, pipe.py - 46, 4, "#64748b")
      circle(ctx, pipe.px + 20, pipe.py - 46, 4, "#64748b")
    }

    // Filter units
    const filter1 = projectIso(6, 6, params)
    rect(ctx, filter1.px - 12, filter1.py - 30, 24, 25, "#334155")
    rect(ctx, filter1.px - 10, filter1.py - 28, 20, 3, "#22c55e")
    rect(ctx, filter1.px - 10, filter1.py - 20, 20, 3, "#22c55e")
    rect(ctx, filter1.px - 10, filter1.py - 12, 20, 3, "#22c55e")

    // Pressure gauges
    circle(ctx, filter1.px, filter1.py - 35, 6, "#1e293b")
    ctx.strokeStyle = "#fbbf24"
    ctx.lineWidth = 2
    ctx.beginPath()
    ctx.arc(filter1.px, filter1.py - 35, 4, Math.PI, Math.PI + Math.PI / 2)
    ctx.stroke()
  }

  // Ventilation Bay (R3) - Vents, air flow indicators, fans
  else if (roomId === "R3") {
    // Large ventilation fans
    for (let i = 0; i < 2; i++) {
      const fan = projectIso(3 + i * 6, 1, params)

      // Fan housing
      circle(ctx, fan.px, fan.py - 40, 18, "#374151")
      ctx.strokeStyle = "#000"
      ctx.lineWidth = 2
      ctx.beginPath()
      ctx.arc(fan.px, fan.py - 40, 18, 0, Math.PI * 2)
      ctx.stroke()

      // Rotating blades
      ctx.save()
      ctx.translate(fan.px, fan.py - 40)
      ctx.rotate(t * 4 + i * Math.PI)
      ...
      for (let p = 0; p < particleCount; p++) {
        const offset = (t * 50 + p * 20) % 100
        const px = fan.px + (Math.random() - 0.5) * 10
        const py = fan.py - 40 + offset
        ctx.fillStyle = `rgba(147, 197, 253, ${1 - offset / 100})`
        ctx.fillRect(px, py, 2, 4)
      }
    }

    // Air quality indicators
    const indicator = projectIso(6, 7, params)
    rect(ctx, indicator.px - 10, indicator.py - 25, 20, 15, "#1e293b")
    ctx.font = "700 10px ui-sans-serif, system-ui"
    ctx.fillStyle = "#22c55e"
    ctx.textAlign = "center"
    ctx.fillText("AIR OK", indicator.px, indicator.py - 15)
    ctx.textAlign = "left"
  }

  // Exit/Purge Gate (EXIT) - Clean room aesthetic, final checkpoint
  else if (roomId === "EXIT") {
    // Clean room indicators
    const clean1 = projectIso(2, 2, params)
    rect(ctx, clean1.px - 15, clean1.py - 30, 30, 25, "#ecfdf5")
    ctx.strokeStyle = "#22c55e"
    ctx.lineWidth = 2
    ctx.strokeRect(clean1.px - 15, clean1.py - 30, 30, 25)

    ctx.font = "700 10px ui-sans-serif, system-ui"
    ctx.fillStyle = "#22c55e"
    ctx.textAlign = "center"
    ctx.fillText("CLEAN", clean1.px, clean1.py - 18)
    ctx.fillText("ZONE", clean1.px, clean1.py - 10)
    ctx.textAlign = "left"

    // Success lights
    for (let i = 0; i < 3; i++) {
      const light = projectIso(5 + i, 1, params)
      const glow = (Math.sin(t * 2 + i) + 1) / 2
      circle(ctx, light.px, light.py - 35, 4, `rgba(34, 197, 94, ${0.5 + glow * 0.5})`)
      ctx.strokeStyle = "#000"
      ctx.beginPath()
      ctx.arc(light.px, light.py - 35, 4, 0, Math.PI * 2)
      ctx.stroke()
    }
  }
}
...
function drawSwitch(
  ...
  // Base
  rect(ctx, px - size / 2, py - size - 10, size, size, "#374151")

  // Button
  const offset = isOn ? 2 : 0
  rect(ctx, px - size / 2 + 2, py - size - 10 + 2 + offset, size - 4, size - 4, color)
  ...
  if (isOn) {
    ...
    ctx.fillStyle = color
    ctx.beginPath()
    ctx.arc(px, py - size / 2 - 10, size, 0, Math.PI * 2)
    ctx.fill()
    ctx.restore()
  }
}

function drawValve(ctx: CanvasRenderingContext2D, px: number, py: number, isOn: boolean, t: number) {
  const radius = 14
  const rotation = isOn ? t * 2 : 0

  // Pipe
  rect(ctx, px - 3, py - 30, 6, 30, "#64748b")
  ...
  // Wheel spokes
  ctx.strokeStyle = isOn ? "#22c55e" : "#94a3b8"
  ctx.lineWidth = 3
  for (let i = 0; i < 6; i++) {
  ...
  // Center
  circle(ctx, 0, 0, 6, isOn ? "#22c55e" : "#475569")
  ctx.strokeStyle = "#000"
  ctx.lineWidth = 1
  ctx.beginPath()
...
function drawDoor(
  ...
  if (orientation === "vertical") {
    if (isOpen) {
      // Door open (swung to side)
      rect(ctx, px + 15, py - 40, 8, 40, doorColor)
      rect(ctx, px + 15, py - 40, 8, 2, frameColor)
    } else {
      // Door closed
      rect(ctx, px - 20, py - 40, 40, 40, doorColor)
      rect(ctx, px - 20, py - 40, 40, 2, frameColor)
      rect(ctx, px - 20, py - 2, 40, 2, frameColor)
      // Door handle
      circle(ctx, px + 10, py - 20, 3, "#fbbf24")
      ctx.strokeStyle = "#000"
      ctx.beginPath()
      ctx.arc(px + 10, py - 20, 3, 0, Math.PI * 2)
      ctx.stroke()
    }
  } else {
    // Horizontal door
    if (isOpen) {
      rect(ctx, px - 4, py - 50, 8, 20, doorColor)
    } else {
      rect(ctx, px - 20, py - 40, 40, 8, doorColor)
      circle(ctx, px, py - 36, 3, "#fbbf24")
    }
  }
}

function drawLED(ctx: CanvasRenderingContext2D, px: number, py: number, isOn: boolean, color: string) {
  const ledColor = isOn ? color : "#374151"
  const radius = 6

  // LED housing
  rect(ctx, px - 8, py - 20, 16, 16, "#1f2937")

  // LED light
  circle(ctx, px, py - 12, radius, ledColor)
  ctx.strokeStyle = "#000"
  ctx.lineWidth = 1
  ctx.beginPath()
  ctx.arc(px, py - 12, radius, 0, Math.PI * 2)
  ctx.stroke()
  ...
  if (isOn) {
    ...
    ctx.beginPath()
    ctx.arc(px, py - 12, radius * 2, 0, Math.PI * 2)
    ctx.fill()
    ctx.restore()
  }
}

function drawPanel(ctx: CanvasRenderingContext2D, px: number, py: number, isActivated: boolean, label: string) {
  const width = 30
  const height = 40

  // Panel body
  rect(ctx, px - width / 2, py - height - 10, width, height, "#1e293b")

  // Screen
  const screenColor = isActivated ? "#22c55e" : "#475569"
  rect(ctx, px - width / 2 + 4, py - height - 6, width - 8, 20, screenColor)

  // Buttons
  const buttonY = py - 20
  for (let i = 0; i < 3; i++) {
    const bx = px - 10 + i * 10
    circle(ctx, bx, buttonY, 3, isActivated ? "#fbbf24" : "#64748b")
    ctx.strokeStyle = "#000"
    ctx.beginPath()
    ctx.arc(bx, buttonY, 3, 0, Math.PI * 2)
    ctx.stroke()
  }
  ...
  if (label) {
    ...
    ctx.fillStyle = "#fff"
    ctx.textAlign = "center"
    ctx.fillText(label, px, py - height - 16)
    ctx.textAlign = "left"
  }
}

function drawConsole(ctx: CanvasRenderingContext2D, px: number, py: number, isUnlocked: boolean) {
  const width = 35
  const height = 30

  // Console base
  rect(ctx, px - width / 2, py - 10, width, 10, "#374151")

  // Screen
  const screenColor = isUnlocked ? "#22c55e" : "#1e293b"
  rect(ctx, px - width / 2 + 2, py - height - 8, width - 4, height - 2, screenColor)
  ...
  if (isUnlocked) {
    ...
    for (let i = 0; i < 4; i++) {
      ctx.beginPath()
      ctx.moveTo(px - width / 2 + 6, py - height + 2 + i * 5)
      ctx.lineTo(px + width / 2 - 6, py - height + 2 + i * 5)
      ctx.stroke()
    }
  }

  // Keyboard
  rect(ctx, px - width / 2, py - 10, width, 4, "#64748b")
}
...
function drawWeightPlate(
  ...
  const color = isPressed ? "#fbbf24" : "#94a3b8"

  // Plate (diamond shape)
  ctx.beginPath()
  ctx.moveTo(px, py + offset)
  ctx.lineTo(px + hw * 0.6, py + hh * 0.6 + offset)
  ctx.lineTo(px, py + tileH * 0.6 + offset)
  ctx.lineTo(px - hw * 0.6, py + hh * 0.6 + offset)
  ctx.closePath()
  ctx.fillStyle = color
  ctx.fill()
  ...
  if (isPressed) {
    ...
    ctx.lineWidth = 2
    ctx.beginPath()
    ctx.moveTo(px - 8, py + hh * 0.6 + offset)
    ctx.lineTo(px + 8, py + hh * 0.6 + offset)
    ctx.stroke()
  }
}

function drawItem(ctx: CanvasRenderingContext2D, px: number, py: number, itemType: string) {
  const size = 16
  ...
  ctx.fillStyle = "#000"
  ctx.beginPath()
  ctx.ellipse(px, py + 2, size * 0.6, size * 0.3, 0, 0, Math.PI * 2)
  ctx.fill()
  ctx.restore()
  ...
  switch (itemType) {
    case "key":
      // Key shape
      circle(ctx, px, py - size - bob, 6, "#fbbf24")
      rect(ctx, px - 2, py - size + 6 - bob, 4, 10, "#fbbf24")
      rect(ctx, px - 2, py - size + 14 - bob, 6, 2, "#fbbf24")
      break
    case "card":
      // Card shape
      rect(ctx, px - 8, py - size - bob, 16, 10, "#3b82f6")
      rect(ctx, px - 6, py - size + 2 - bob, 4, 2, "#fbbf24")
      break
    default:
      // Generic item
      circle(ctx, px, py - size - bob, 8, "#8b5cf6")
  }
...
function drawInteractionPrompt(ctx: CanvasRenderingContext2D, px: number, py: number, objectName: string) {
  const text = `Press E: ${objectName}`
  ctx.font = "700 12px ui-sans-serif, system-ui"
  const metrics = ctx.measureText(text)
  const padding = 8
  ...
  ctx.beginPath()
  ctx.roundRect(px - width / 2, py - height, width, height, 4)
  ctx.fill()
